...
SP_PRIV enum sp_return list_ports_fallback(struct sp_port ***list)
{
	...
	while ((entry = readdir(dir))) {
		...
		if (strncmp(entry->d_name, "ttyUSB", 6) == 0 ||    // USB serial devices
			...
			// Skip device opening test to avoid SELinux lock errors on iMin devices
			// The device will be opened later when actually needed
			LOGI("Skipping device open test for %s (SELinux-safe)", name);
